...
@Component({
  selector: 'app-re-cassation-response',
  templateUrl: './re-cassation-response.component.html',
  styleUrls: ['./re-cassation-response.component.scss'],
})
export class ReCassationResponseComponent implements OnInit, OnDestroy {
  ...
  submit(actionId: number) {
    ...
    this.lawsuitService
      .apiFetch(data, 'law/add/reCassationResponse', actionId)
      .subscribe(
        (action) => {
